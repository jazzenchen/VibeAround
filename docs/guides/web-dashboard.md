...
- **Dev server preview.** Register a local port and a local owner iframe loads that server's loopback origin directly, preserving the app's relative APIs, fetch, WebSockets, and HMR behavior. A tunneled owner requires browser pairing. With a public tunnel, the Preview also gets a 10-minute code-gated Share; its first transport proxies only GET/HEAD documents and static assets, not APIs, writes, workers, WebSockets, or HMR. Agents create these via the `va-preview` skill / MCP `preview` tool ([tool reference](../reference/api-surfaces.md#mcp-tools)).
