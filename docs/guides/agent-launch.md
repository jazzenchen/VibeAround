...
Agent Launch opens a coding agent CLI in your terminal with materialized credentials, model routing, synchronized project skills, and MCP config. See the [launch subsystem internals](../internals/launch.md).
...
2. **Workspace preparation.** VibeAround removes known old VibeAround skill names and writes the current bundled project skills on every launch. If `auth.json` carries an MCP credential, it also writes it into the agent's project config. A missing credential skips only MCP config; it never skips skill sync or deletes project files.
