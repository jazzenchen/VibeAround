...
Another VibeAround instance (or a stale process from a crash) holds it. `va status` tells you if a healthy daemon is answering; otherwise find and kill the holder. Child processes die with their daemon, but a previous *daemon* process must be stopped by you.
