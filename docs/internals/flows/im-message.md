...
**Epilogue.** After the turn, `PromptDone` turns the typing indicator off and the host remains warm; errors are sent as `❌` system text (auth errors auto-close the thread). There is no per-turn idle-shutdown timer. Separately, after a genuinely new host starts, an over-limit pool may evict at most one eligible least-recently-active thread; any thread with resident subagents is ineligible, and no candidate means soft overflow.
