...
**7. Ensure agent + session.** `ThreadRuntime` keeps durable session identity separately from an `AcpSessionRunner`, which owns the live `Agent` and handler for one generation. A dead generation is replaced as a unit. Agent spawn is registered with the supervisor (`Never` restart policy); a cancellation-safe pending owner holds that registration until ACP initialize succeeds, so Stop/abort during initialize unregisters and reaps the child. The runtime then creates or resumes the recorded CLI session. IM route attachments are rehydratable but do not replay old output: warm-pool eviction preserves the same runtime/thread/profile/session and the next message resumes it.
...
**9. Notifications → outputs.** Every ACP `session_notification` from the host agent is wrapped as a thread reply and fanned out to the routes attached to the thread. For stdio plugins this wrapper is sent as the VibeAround extension notification `va/thread_reply`; it is not the standard ACP `session/update` wire method. The active origin is included even if attachment state changed mid-turn. Only that origin carries the ephemeral `replyTo`; other attached surfaces receive the live thread event without pretending to reply to the inbound platform message. The stdio prompt path forwards these notifications before returning the matching ACP prompt response, after which the SDK can seal and drain renderer delivery. The SDK keys renderer and delivery state by the full `ChannelTarget` `(instance, actor, chat, topic, replyTo)`.
→ `src/core/src/channels/bridge_handler.rs` (`session_notification`)

**10. Output → chat.** `PluginHost` routes each output by `channel_instance_id` to the current live plugin runtime. One in-memory FIFO per plugin generation keeps output and its completion barrier in wire order; it is currently unbounded and does not provide backpressure. No IM output is persisted or replayed. If the runtime is absent or disconnects, enqueueing fails; an undeliverable permission request is cancelled so the agent cannot hang.
→ `src/core/src/channels/plugin_host.rs` (`send_output`), `plugin_runner.rs`

**Epilogue.** After the turn, Web/TUI consumers receive `TurnStatus { active: false }`; stdio plugins finish from the notification-before-response boundary described above. The host remains warm, and errors are sent as `❌` system text (auth errors auto-close the thread). There is no per-turn idle-shutdown timer. Separately, after a genuinely new host starts, an over-limit pool may evict at most one eligible least-recently-active thread; any thread with resident subagents is ineligible, and no candidate means soft overflow.
→ `src/core/src/channels/prompt/handler.rs` (post-start reconciliation), `workspace/manager_routes.rs` (warm-thread selection)
