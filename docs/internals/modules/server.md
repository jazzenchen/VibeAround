...
3. **Shutdown order matters** (`RunningDaemon::stop`): stop accepting Web/input → drain `ConversationIngress` → stop channel-owned processes → stop workspace hosts/search → safety-net registry kill → previews/listeners. A queued prompt must never run after workspace teardown.
