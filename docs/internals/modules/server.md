...
3. **Shutdown order matters** (`RunningDaemon::stop`): stop accepting Web/input → drain `ConversationIngress` → stop channel-owned processes → stop workspace hosts/search → previews → listeners and the tunnel/web tasks → supervisor `shutdown_all` last, once nothing can spawn any more (an in-process hot restart relies on this step; the OS-level lease only covers a dying process). A queued prompt must never run after workspace teardown.
