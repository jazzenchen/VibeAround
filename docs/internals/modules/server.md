...
1. **Route protection layout**: everything is token-gated except the deliberate open set (SPA shell/assets, code-gated Preview Shares, pairing entry). Owner Preview routes require loopback/token access or remote pairing. Server Shares may proxy only GET/HEAD documents and static assets; APIs, writes, workers, WebSockets, and HMR stay closed. New routes default to protected; adding an open route is a security-model change.
