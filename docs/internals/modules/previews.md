...
- **← server (MCP `va_mcp_preview`):** agents pass either a dev-server port or a Markdown file; the `va-preview` skill wraps the unified tool.
...
4. **Refresh has one explicit agent trigger**: finishing an agent turn does not refresh the iframe. A successful MCP `va_mcp_preview` call refreshes the matching open owner Preview without prompting. A user-triggered Preview refresh asks for confirmation only when it will clear review drafts.
