...
**在浏览器里操作一切。** Web 控制台提供真正的终端（xterm over WebSocket）、带权限卡片的 Web Chat、dev server 实时预览和 Markdown 渲染预览 —— 默认只在本机，也可以通过内置隧道（ngrok、localtunnel 或 Cloudflare）从任何地方访问，由配对码保护。
