...
2. **Workspace 准备。** 每次启动都会删除已知的旧 VibeAround skill 名称，并写入当前 bundled 项目级 skills。若 `auth.json` 里有 MCP credential，同时把它写入 Agent 的项目配置；credential 缺失时只跳过 MCP 配置，不会跳过 skill sync，也不会删除项目文件。
