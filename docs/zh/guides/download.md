...
当前稳定桌面版本是 [VibeAround v0.7.19](https://github.com/jazzenchen/VibeAround/releases/tag/v0.7.19)，发布时间为 2026-07-22。
...
v0.7.19 修复 Channel Catalog 的发布断层：内置的 9 个 Channel 插件现已全部指向经过校验的 0.6.7 `main` 版本，因此已安装用户可以收到插件中已经发布的权限、投递、附件、长消息和重连修复。桌面发布流程现在会在构建前拒绝过期的 catalog pin。

命令行发行包单独发布为 [VibeAround CLI 0.0.8](https://github.com/jazzenchen/VibeAround/releases/tag/va-v0.0.8)。需要桌面应用时用上面的安装包；terminal-first 安装用 CLI release 里的资产。如果偏好 npm，可以这样安装当前 CLI：
