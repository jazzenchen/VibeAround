...
当前稳定桌面版本是 [VibeAround v0.7.18](https://github.com/jazzenchen/VibeAround/releases/tag/v0.7.18)，发布时间为 2026-07-21。
...
v0.7.18 新增 macOS 与 Windows 的 ChatGPT 桌面应用发现和启动支持，同时保持现有 `codex-desktop` 目标兼容。Channel 插件输出现在通过一条有序、无静默丢失的 FIFO 完成 Host 到插件的最后一跳，运行日志也不再持久化频道消息正文。
