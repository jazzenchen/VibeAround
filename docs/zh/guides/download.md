...
当前稳定桌面版本是 [VibeAround v0.7.25](https://github.com/jazzenchen/VibeAround/releases/tag/v0.7.25)，发布时间为 2026-08-30。
...
v0.7.25 修复了 0.7.23 引入的多模态图片解析器的协议处理。现在会完整保留所选的 Profile、API 类型和模型，并针对 OpenAI Chat Completions、OpenAI Responses、Anthropic Messages 和 Gemini GenerateContent Profile 使用对应协议的 endpoint、认证、请求编码和响应解析。OpenAI Responses Profile 的连接测试也改用了有效的输出 token 上限。现有配置无需迁移。
