...
- **← server (MCP `va_mcp_preview`)：** agents 传入 dev-server port 或 Markdown file；`va-preview` skill 包装这个统一工具。
...
3. **Preview 状态一律不恢复**：File 与 Server Preview 状态只存在于内存。最小 cleanup journal 只记录 Server port，用于退出中断后，让下一次启动重复关闭时的同一套 cleanup。Cleanup 会杀掉这些 port、删除 journal，绝不重建 Preview；关闭 thread/session 不处理 Preview。
4. **Agent 侧只有一个明确的刷新触发点**：agent 回合结束不会刷新 iframe。MCP `va_mcp_preview` 调用成功后，无提示刷新当前已打开的对应 owner Preview；用户手动刷新 Preview 时，只有存在将被清空的 review draft 才会确认。
