...
3. **Shutdown order matters**：先停 Web/input ingress，再 drain `ConversationIngress`，然后停止 channel、workspace hosts、search，最后 safety-net registry kill、previews 和 listeners。teardown 后不能再执行排队 prompt。
