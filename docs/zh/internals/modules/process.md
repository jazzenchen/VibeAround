...
- **← server：** daemon shutdown 调 `shutdown_all`（同进程内热重启也走这里，OS 级 lease 在那种情况下不会触发）；daemon start 不再做任何事。
...
- Windows 的 Job Object 路径是在没有 Windows 机器的情况下写的，还需要在 Windows 上跑一遍 `cargo test -p common process::` 并对真实 daemon 做一次强杀验证。
