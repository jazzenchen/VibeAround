...
**Operate everything from a browser.** The web dashboard serves a real terminal (xterm over WebSocket), a web chat with permission cards, local-only dev-server previews, and rendered Markdown previews. The dashboard and Markdown owner previews can also be reached through a built-in tunnel (ngrok, localtunnel, Cloudflare, or Tailscale Funnel) after pairing; a Markdown share instead uses its own reusable six-digit access code and 10-minute deadline.
