...
**Agent-as-API (`local-agent`).** Turns a hosted coding agent itself into an OpenAI/Anthropic-compatible endpoint: requests become prompts to a real agent (with its tools and workspace), responses stream back in the requested dialect. This lets any OpenAI-compatible tool drive a full coding agent. Access is opt-in per agent (`local_agent_api.agents` on top of the service switch; the direct profile is refused), and requests that identify a conversation — a chained `previous_response_id`, or the `x-vibearound-conversation` header — share one persistent ACP session: only the new tail of the transcript is prompted, while new or lost conversations are seeded from the transmitted history inside an explicit replay envelope. The bridge keeps no history of its own and never verifies the client's copy; the backend session owns the conversation, requests on a busy conversation displace the in-flight turn, and idle agent processes are shut down (sessions stay resumable) then forgotten. See [API surfaces](../reference/api-surfaces.md) for the wire contract.
