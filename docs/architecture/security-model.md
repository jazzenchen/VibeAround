...
A Share does not use owner pairing: its opaque URL opens an access-code gate, and successful verification issues a Secure, HttpOnly browser grant. The URL, reusable six-digit code, and grant belong to one Preview and expire together after 10 minutes. A Server Share revalidates that grant on every proxied request and forwards only GET/HEAD iframe navigations and browser-declared static subresources. Browser fetch/XHR/EventSource requests, workers, non-GET/HEAD methods, WebSocket/HMR traffic, and forwarding `/va/*` to the child are rejected or unsupported; the Share grant does not include owner chat or review. This is a page-preview transport, not general API compatibility or an API-isolation sandbox: an accepted navigation or subresource can use any path. Everything else on the tunnel still requires pairing + token.
