...
#[tokio::test]
async fn forked_session_creates_a_new_web_thread_without_moving_the_source_route() {
    let (workspaces, threads, attachments) = temp_paths();
    let manager = WorkspaceThreadManager::with_paths(workspaces, threads, attachments);
    let root = std::env::temp_dir().join(format!("vibearound-ws-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&root).unwrap();
    let source_route = RouteKey::new("web", "source-chat");
    let source_thread_id = seed_session_thread(
        &manager,
        root,
        "claude",
        Some("direct"),
    ...
    .await;
    manager
        .attach_thread(&source_route, &source_thread_id)
        .await
        .unwrap();
    let source_before = manager.thread(&source_thread_id).await.unwrap().unwrap();

    let forked = manager
        .create_web_thread_for_fork(ThreadSessionFork {
            session_id: "forked-session".to_string(),
            workspace_id: source_before.workspace_id.clone(),
            host_binding: source_before.host_binding.clone(),
        })
        .await
        .unwrap();
    let state = forked.state().await;

    assert_ne!(state.thread_id, source_thread_id);
    assert_eq!(state.host_binding, source_before.host_binding);
    assert_eq!(state.session_id.as_deref(), Some("forked-session"));
    assert_eq!(
        manager.thread(&source_thread_id).await.unwrap().unwrap(),
        source_before
    );
    assert_eq!(
        manager
            .current_attachment(&source_route)
            .await
            .unwrap()
            .unwrap()
            .thread_id,
    ...
    let forked_route = web_route_for_thread(&state.thread_id);
    assert_eq!(
        manager
            .current_attachment(&forked_route)
            .await
            .unwrap()
            .unwrap()
            .thread_id,
...
#[tokio::test]
async fn fork_failure_does_not_create_a_child_thread() {
    let (workspaces, threads, attachments) = temp_paths();
    let manager = WorkspaceThreadManager::with_paths(workspaces, threads, attachments);
    let root = std::env::temp_dir().join(format!("vibearound-ws-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&root).unwrap();
    ...
    .await;
    let before = manager.thread_store.read_events().await.unwrap();

    let error = match manager.fork_thread_to_web(&source_thread_id).await {
        Ok(_) => panic!("fork should require a live source agent"),
        Err(error) => error,
    };

    assert_eq!(error.to_string(), "session fork requires a live agent");
    assert_eq!(manager.thread_store.read_events().await.unwrap(), before);
}
