...
Read `$VIBEAROUND_THREAD_ID` when present. Also use the `va-session` skill to resolve the current session ID. Prefer `thread_id`; otherwise pass the current VibeAround agent ID as `agent_kind` together with `session_id` so the Preview child is linked to the current task.
