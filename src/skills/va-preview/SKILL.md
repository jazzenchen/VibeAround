...
Read `$VIBEAROUND_THREAD_ID` when present. Also use the `va-session` skill to resolve the current session ID. Prefer `thread_id`; otherwise pass the current VibeAround agent ID as `agent_kind` together with `session_id` so VibeAround can link the Preview child and clean up the dev server.
...
Arguments:
  ...
  agent_kind: "<current VibeAround agent ID>"  (pass with session_id when thread_id is unavailable)
