...
Pass `$VIBEAROUND_THREAD_ID` when present. If an exact current session ID is readily available, pass it with `agent_kind: "gemini"`. Both are optional; do not delay or block Preview when either is unavailable.
...
Arguments:
  ...
  agent_kind: "gemini"  (pass with session_id)
