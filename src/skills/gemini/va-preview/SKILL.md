...
Pass `$VIBEAROUND_THREAD_ID` when present. If an exact current session ID is readily available, pass it with `agent_kind: "gemini"` for earlier lifecycle cleanup. Both identities are optional: do not delay or block Preview when either is unavailable; VibeAround creates a standalone Preview conversation.
...
Arguments:
  ...
  agent_kind: "gemini"  (pass with session_id when available)
