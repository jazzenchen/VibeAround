---
name: va-preview
description: "VibeAround Agent only: start a VibeAround Preview for either a running local web server or a Markdown file. Use after starting a dev server, after creating or updating Markdown, or when the user asks to preview a browsable artifact. Only available when the VibeAround MCP server is connected."
---
...
Pass `$VIBEAROUND_THREAD_ID` when present. If an exact current session ID is readily available, pass it with `agent_kind: "va-agent"`. Both are optional; do not delay or block Preview when either is unavailable.
...
Arguments:
  ...
  agent_kind: "va-agent"  (pass with session_id)
