...
async fn owner_shell_groups_previews_without_rendering_share_credentials() {
    ...
    assert!(body.contains("id=\"preview-comment-popover\""));
    assert!(body.contains("id=\"preview-review-chips\""));
    assert!(body.contains("id=\"preview-select-element\""));
    assert!(body.contains("id=\"preview-chat-refresh\""));
    assert!(body.contains("new WebSocket(socketUrl(option.value))"));
    assert!(body.contains("event.source === frame.contentWindow"));
    assert!(body.contains("window.VAPreviewComments"));
    assert!(body.contains("window.VAPreviewFrameReview"));
    assert!(body.contains("window.VAPreviewChat"));
    assert!(body.contains("Source line: "));
    assert!(body.contains("va-preview-turn-complete"));
    assert!(!body.contains("Comment 1"));
    assert!(!body.contains("preview-review-panel"));
